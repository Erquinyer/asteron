...
const estadoConfig = {
  ...
  en_mantenimiento: { label: 'En mantenimiento',  style: 'bg-amber-100 text-amber-700'   },
  sin_asignar:      { label: 'Sin asignar',       style: 'bg-blue-100 text-blue-600'     },
  guardada:         { label: 'Guardada',          style: 'bg-purple-100 text-purple-600' },
  dado_de_baja:     { label: 'Dado de baja',      style: 'bg-red-100 text-red-600'       },
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg my-4">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h3 className="text-base font-semibold text-slate-800">{maquina ? 'Editar equipo' : 'Nuevo equipo'}</h3>
  ...
  return (
    <div className="space-y-5 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-800">Maquinaria e inventario</h1>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {lista.map(m => {
                      ...
                      return (
                        <tr key={m.id_maquina} className="hover:bg-slate-50 transition-colors">
                          <td className="px-6 py-4 font-mono text-xs text-slate-500">{m.codigo || '—'}</td>
                          <td className="px-6 py-4">
                            <p className="font-medium text-slate-800">{m.nombre}</p>
                            {m.descripcion && <p className="text-xs text-slate-400 truncate max-w-[200px]">{m.descripcion}</p>}
                          </td>
                          ...
                          <td className="px-6 py-4 text-slate-500 text-xs hidden lg:table-cell">{m.ubicacion || '—'}</td>
                          <td className="px-6 py-4">
